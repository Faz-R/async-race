...
import {
  getCars, startEngine, stopEngine, drive, getWinners,
} from './api';
...
const renderGarage = () => `
  <ul class='garage-list'>
    ${store.cars.map((car: { id: number; name: string; color: string; isEngineStarted: boolean; }) => `
    <li>${renderCar(car)}</li>
    `).join('')}
  </ul>
`;
...
const startDriving = async (id: number, isRace?: boolean) => {
  ...
  if (<HTMLButtonElement>document.getElementById(`stop-engine-car-${id}`)) {
...
const stopDriving = async (id: number) => {
  ...
  if ((<HTMLButtonElement>document.getElementById(`start-engine-car-${id}`))) {
